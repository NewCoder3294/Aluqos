    ...
    >
      <Card>
        <CardContent className="flex items-baseline gap-5">
          {stat.kind === "count" ? (
            <Serif className="text-[44px] tracking-[-0.02em] text-ink leading-none shrink-0 tabular-nums">
              <CountUp
                from={stat.from}
                to={stat.to}
            ...
            <motion.div
              ...
              className="shrink-0"
            >
              <Serif className="text-[44px] tracking-[-0.02em] text-ink leading-none">
                {stat.value}
              </Serif>
            </motion.div>
          )}
          <div className="min-w-0">
            <div className="text-[15px] leading-[1.5] text-ink-muted">
              {stat.label}
            </div>
            <div className="mt-1 text-[11px] uppercase tracking-[0.12em] text-ink-faint">
              {stat.caption}
            </div>
          </div>
        </CardContent>
    ...
    <section id="product" className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-10 lg:gap-16 items-start">
        <div className="space-y-5">
          <Reveal>
            <p className="serif text-[20px] leading-[1.55] text-ink">
              Every AI tool today puts the burden on the human.
            </p>
          </Reveal>
          <Reveal delay={0.05}>
            <p className="text-[16px] leading-[1.7] text-ink-muted max-w-[56ch]">
