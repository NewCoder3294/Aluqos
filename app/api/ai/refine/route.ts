...
import { RefineInputSchema, parseJsonRequest } from "@/src/ai/prompts/input-schemas";
...
export async function POST(req: NextRequest) {
  ...
  const parsed = await parseJsonRequest(req, RefineInputSchema);
  if (!parsed.ok) return parsed.res;
  const { system, user } = buildRefinePrdPrompt(parsed.data);
